...
public:
    ...
    // Xazab Specific WalletInitInterface InitPrivateSendSettings
 ...
 * between xazabd, and xazab-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"Xazab Core - \""));
    ...
    strUsage += HelpMessageOpt("-algo=<algo>", "Mining algorithms: sha256d, scrypt, x11");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/xazab/xazab>";
    const std::string URL_WEBSITE = "<https://xazab.org>";
...
/** Sanity checks
 *  Ensure that Xazab Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Xazab Core version %s\n", FormatFullVersion());
}
...
{
    // Make sure only a single Xazab Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if Xazab Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Xazab Core is started while in a temporary directory.\n",
        ...
        do {
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/xazab/xazab/pull/1817 and https://github.com/xazab/xazab/pull/1743
    ...
    // ********************************************************* Step 10c: schedule Xazab-specific tasks
