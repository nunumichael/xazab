...
const char * const BITCOIN_CONF_FILENAME = "xazab.conf";
const char * const BITCOIN_PID_FILENAME = "xazabd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "xazabd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XazabCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XazabCore
    // Mac: ~/Library/Application Support/XazabCore
    // Unix: ~/.xazabcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XazabCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XazabCore";
#else
    // Unix
    return pathRet / ".xazabcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty xazab.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Xazab Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Xazab Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Xazab Core developers";
    }
