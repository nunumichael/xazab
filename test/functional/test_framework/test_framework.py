...
class BitcoinTestFramework():
    ...
    def stop_node(self, i, wait=0):
        """Stop a xazabd test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple xazabd test nodes"""
        for node in self.nodes:
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in xazab.conf
...
class XazabTestFramework(BitcoinTestFramework):
    def set_xazab_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
